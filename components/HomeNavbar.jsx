    ...
    <nav className="bg-white max-w-[90%] mx-auto px-6 py-6">
      <div className="flex justify-between items-center">
        <div className="flex items-center">
          ...
          <div className="hidden md:flex space-x-10 ml-[4rem]">
            {navLinks.map((link) => (
              <Link key={link.label} href={link.href} className="text-[#2E8B57] text-lg transition-colors duration-200">
                {link.label}
              </Link>
            ))}
          </div>
      ...
      {isOpen && (
        <div className="md:hidden flex flex-col items-start bg-white py-4 space-y-8">
          ...
          <Link href="/auth/login" className="text-[#2E8B57] py-2 border-b w-full transition-colors duration-200">
            Log in
          </Link>
          <Link href="/auth/register" className="text-[#2E8B57] py-2 border-b w-full transition-colors duration-200">
            Sign Up
          </Link>
        </div>
      )}
